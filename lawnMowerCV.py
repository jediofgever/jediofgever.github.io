...
def blink(pin):                        #Blink leds on Raspberrry Pi to see that software is working properly
...
ser = serial.Serial('/dev/ttyACM0', 115200, timeout=5) # set arduino serial comm.
time.sleep(1.5)
ser.flush()
...
while True:
    ...
    f, frame = capture.read()                           #upper cam
    ...
    f, frame1 = capture1.read()                            #below cam
    ...
    for j in range (0,imggreen_capture1.shape[1],StepSize):         
        for i in range(imggreen_capture1.shape[0]-5,0,-1):         #step through every pixel in height of array from bottom to top
                                              			   #Ignore first couple of pixels as may trigger due to undistort
            if imggreen_capture1.item(i,j) == 0: 	           
                EdgeArray_below.append((j,i))     		   
                break                           	            
        else:                                			    
            EdgeArray_below.append((j,0))         		    
